...
import { deleteEvent } from "@/app/lib/actions/event.action"
import { FaLocationArrow, FaCalendarAlt } from "react-icons/fa"
import { formatDateTime } from "@/app/lib/dateformat"
import Image from "next/image"

export default function EventDetail({event, userId}){
    const {title, category, description, imageUrl, location, startDateTime, endDateTime, url, organizer} = event
            ...
            <div className="w-full md:w-1/2 h-90 bg-slate-100 p-6 rounded-r">
                <div className="flex flex-col gap-4">
                    <div className="flex flex-col gap-4">
                        ...
                        <p>{description}</p>
                        <a href={url} className="btn btn-accent">Visit Event Website</a>
                        {
